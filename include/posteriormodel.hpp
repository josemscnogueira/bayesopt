...
namespace bayesopt {
 ...
 /**
  ...
  public:
    ...
    // Unscented Outcome functions
    double  getValueUnscented(         const vectord& x);
    double  getValueAtMinimumUnscented(void);
    vectord getPointAtMinimumUnscented(void);
  ...
  { return (mParameters.unscented_outcome ? this->getPointAtMinimumUnscented() : mData.getPointAtMinimum()); };

  inline double PosteriorModel::getValueAtMinimum()
  { return (mParameters.unscented_outcome ? this->getValueAtMinimumUnscented() : mData.getValueAtMinimum()); };
